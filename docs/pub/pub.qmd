...
<!-- NOTE: Ajouter source et dire que la majorité des données d'entrainement de LLAMA c'est English Touvron ? Et ajouter le tableau?-->
...
Indeed, the majority of LLMs are trained primarily on English data, raising questions about their performance in other languages.

This paper addresses a fundamental question in this domain: Can open and closed source general purpose foundational LLMs accurately evaluate sentiment in non-English texts?^[While the term 'open weights' technically distinguishes models releasing only parameters from fully 'open-source' models (which often include training code and potentially data), this study uses 'open-source' as a broader category for simplicity and common understanding. It refers to models offering greater accessibility and transparency via publicly available weights compared to closed-source alternatives. The reader should note this distinction, as many models evaluated in our 'open-source' group technically meet the 'open weights' definition.]
...
Sentiment analysis serves as an ideal test case for examining cross-linguistic capabilities of LLMs for several reasons. 
...
This study first evaluate the performance of leading open and closed weights foundational LLMs in sentiment analysis with french texts. 
It then examine how prompt language affects performance, identify cross-linguistic patterns, and analyze model behavior across french and english. 
...
We evaluate whether foundational models demonstrate sufficient cross-linguistic capabilities for sentiment analysis without task-specific fine-tuning, comparing their performance to traditional easy-to-use dictionary-based methods and examining the impact of prompt language on analytical outcomes.
...
A major limit of this project is that the manual annotation was done by a single coder with a small sample of sentences.
...
To establish a classical baseline sentiment measures, we employed lexicon-based methods on both the original French text and its English translation. 
...
Second, regarding the influence of prompt language, our findings present a more nuanced picture than suggested by some prior research [@Jiang2020How; @Lin2022Few; @Muennighoff2023Crosslingual]. 
While the condition using English prompts on French text (EN→FR) frequently ranked highest across correlation, MAE, and F1 metrics, its performance advantage over using French prompts on French text (FR→FR) was marginal and not statistically significant. 
This suggests that, at least for French sentiment analysis with the current generation of models, a strong universal benefit from English prompting may not hold. 
Both native-language (FR→FR) and cross-language (EN→FR) prompting strategies yielded comparable results. 
Notably, analyzing English translations with English prompts (EN→EN) tended to perform slightly worse than direct analysis of the French text (FR→FR or EN→FR), potentially indicating minor information loss or distortion introduced during the translation process itself, despite using a high-quality translation service.
...
Fourth, the marked difference in performance between 7-category and 3-category classification across all models is significant. 
...
Implications: Practically, these results suggest social scientists can confidently employ leading closed source LLMs, and cautiously use larger open source LLMs, for French sentiment analysis without needing fine-tuning expertise or extensive computational resources. 
The choice between French or English prompts appears less critical than previously thought for this specific task and language, slightly favoring direct analysis of the original French text. 
Theoretically, our findings contribute to the understanding of LLM cross-lingual capabilities, suggesting the "English prompt advantage" may be task-dependent or language-pair specific, and warrants further investigation.

# Limitations

Several specific limitations warrant careful consideration when interpreting the findings of this study. 
The foundation of the evaluation, the ground truth dataset, is constrained primarily by the use of a single annotator for the manual coding process.
This prevents the calculation of inter-annotator agreement, making it impossible to quantify the reliability of the annotations and leaving the benchmark potentially susceptible to individual subjectivity and bias. 
Furthermore, the limited validation sample size of 200 sentences, though randomly drawn, constitutes only a small fraction of the extensive corpus and may not adequately capture the full spectrum of linguistic variations and sentiment nuances present across thousands of articles. 

The generalizability of the results is significantly constrained by the corpus specificity, particularly its language focus exclusively on French. 
...
Our findings address the core research questions affirmatively: foundational LLMs can evaluate sentiment accurately in French. 
...
Open-source models, especially larger ones, also proved capable, presenting viable options.
...
Furthermore, analysis across the entire corpus reveals that the overall sentiment variations captured by LSD closely mirror the temporal trends identified by leading LLMs like Gemini, as illustrated graphically in Figure 
...
Regarding LLM-specific findings, the anticipated significant advantage of English prompting was not observed for French sentiment analysis in this study; native-language prompting was comparably effective. 
