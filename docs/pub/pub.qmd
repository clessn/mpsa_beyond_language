...
Recent research has uncovered a compelling phenomenon: prompting in English often improves performance in cross-linguistic tasks, even when the target language is non-English [@Jiang2020How; @Lin2022Few; @Muennighoff2023Crosslingual; @Nie2024Decomposed]. These findings suggest a fundamental characteristic of current LLM architectures that merits deeper investigation.

While specialized models like BERT and its variants have demonstrated superior performance through task-specific fine-tuning [@Anirban2021Sentiment; @Michailidis2024Comparative; @Rafeef2023Evaluation], their implementation requirements may limit accessibility for many social science researchers. Such models typically require technical expertise in machine learning workflows, substantial computational resources, and domain-specific labeled datasets [@Laurer2023Less; @Chalkidis2020LEGAL; @Strubell2019Energy]. In contrast, general-purpose foundational LLMs accessible through APIs offer a more inclusive approach, requiring primarily prompting skills rather than extensive technical infrastructure [@Perron2024Demystifying; @Ghosh2023Navigating].
...
This study evaluates the performance of leading open and closed-source foundational LLMs in sentiment analysis with French texts and examines how prompt language affects performance. Our findings contribute to understanding LLM capabilities and limitations in multilingual contexts while offering practical guidelines for their deployment across diverse language settings.
...
Figure 1 shows the temporal distribution of articles in our corpus, revealing significant variation in coverage over time, with notable increases in the early 2000s corresponding to major developments in the open-source ecosystem.
...
Figure 2 presents the correlation analysis results, demonstrating the varying performance of different models and conditions in capturing sentiment patterns relative to manually annotated ground truth. This approach was chosen to evaluate the models' ability to accurately reflect overall sentiment intensity and direction in a continuous manner. 
...
The mean absolute error (MAE) followed a similar pattern, with French prompts on French text achieving the lowest error (MAE = 0.235), English prompts on French text slightly higher (MAE = 0.237), and English prompts on English text showing the highest error (MAE = 0.228). Note that lower MAE values indicate better performance, so the MAE = 0.228 for English prompts on English text actually represents the best performance in this metric, contrary to what might be expected.
...
Traditional dictionary-based approaches provided reasonable baseline performance, particularly for simplified 3-category classification, where the English Lexicoder Sentiment Dictionary achieved an F1 score of 0.625. When compared to our focused analysis of the 9 top scoring LLMs, this performance was competitive but generally lower than the average LLM. The focused LLM analysis showed mean F1 scores ranging from 0.606 to 0.640 across language conditions, indicating that contemporary general-purpose LLMs have surpassed traditional dictionary methods for 3-category classification.

For correlation metrics, the English dictionary (r = 0.531) performed lower than the focused LLM analysis average (mean r = 0.599 across conditions), with top-performing models like DeepSeek Chat achieving correlations exceeding 0.800. This substantial performance gap suggests that modern LLMs provide superior capability for capturing sentiment intensity and nuance compared to lexicon-based approaches, while dictionary methods remain valuable for their transparency and interpretability.
...
The average F1 score across all models improved substantially when moving from 7 to 3 categories, with the best models reaching F1 scores of 0.780 for 3-category classification compared to 0.451 for 7-category classification. Figure @fig:f1-scores illustrates these performance differences across models and classification schemes.
...
Figure 3 illustrates the temporal sentiment trends as captured by both Gemini (our top-performing LLM) and the Lexicoder Sentiment Dictionary, demonstrating their parallel performance in identifying sentiment patterns over time.
